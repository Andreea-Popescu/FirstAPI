...
public async Task <IActionResult> UploadCharacterImage ([FromForm] IFormFile file)
    ...
    // POST pt upload fisiere api/upload/aeon-image
    [HttpPost("aeon-image")]
    public async Task <IActionResult> UploadAeonImage ([FromForm] IFormFile file)
        {
            // verificam sa nu fie gol fisierul
            if (file == null || file.Length == 0)
            {
            ...
            var uniqueFileName = $"{Guid.NewGuid()}{extension}"; // dam un nume unic fisierului incarcat
            var uploadsFolder = Path.Combine(_environment.WebRootPath ?? Path.Combine(Directory.GetCurrentDirectory(), "wwwroot"), "images", "aeons");
            // spunem sa salveze fisieru pe calea wwwroot/images/aeons
            if (!Directory.Exists(uploadsFolder))
            {
                Directory.CreateDirectory(uploadsFolder); // dc nu exista, sa l creeze
            }
            ...
            // returnam calea relativa spre fisier
            var relativeUrl = $"/images/aeons/{uniqueFileName}";
            return Ok(new { url = relativeUrl });
        }
}